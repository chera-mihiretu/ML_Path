...
def convertToPixels(oneDimensionImagePixel:List[int], width:int=28, height:int=28, threshold:int=187) -> List[List[int]]:
    ...
    for row in range(width):
        for col in range(height):
            ...
            # print('GREY' ,gray, 'INSTANCE',isinstance(oneDimensionImagePixel[row * width + col], list))
            if isinstance(oneDimensionImagePixel[row * width + col], list) or isinstance(oneDimensionImagePixel[row * width + col], tuple) :
